        ...
        */
        ...
        messageHandler: function(data, context){
            ...
            $.each(response.find("message"),function(i,element){
                try{
                    var e = $(element);
                   // console.log($(element).find('delay').attr('stamp'));
                    xmpp.onMessage({from: e.attr("from"), body: e.find(".body").html(),stamp:$(e).find('delay').attr('stamp'),attributes:e[0].attributes,data:response.children()});
                }catch(e){}
            });
