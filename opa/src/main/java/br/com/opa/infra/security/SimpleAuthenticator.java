...
import br.com.opa.service.security.SecurityBean;
...
public class SimpleAuthenticator extends BaseAuthenticator {
    ...
    private @Inject SecurityBean securityBean;
    ...
    public void authenticate() {
    	try{
    		if(customIdentity.getUsuario() != null && customIdentity.getUsuario().isAcessoPublico()){
    			Usuario usuario = usuarioService.login(customIdentity.getUsuario().getLogin(),customIdentity.getUsuario().getSenha());
    			setAccount(new User(usuario.getLogin()));
    			...
    			this.customIdentity.setMenuCollapse(false);
    			setStatus(AuthenticationStatus.SUCCESS);
//    			this.customIdentity.getNsaChatBean().connect("paulo", "ev1235");
    		}
    	}catch(Exception e){
    ...
    public String logout() {
        identity.logout();
//        this.customIdentity.getNsaChatBean().disconnect();
